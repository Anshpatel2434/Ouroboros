...
def generate(spec: ProjectSpec, deps: GeneratorDeps | None = None) -> GenerationResult:
    ...
    for attempt in range(1, deps.max_attempts + 1):
        backlog = plan_backlog(deps.llm, deps.retriever, spec, feedback=feedback)
        skeleton = plan_skeleton(deps.llm, deps.retriever, spec, feedback=feedback)
